...
NC='\033[0m' 
VERSAO=11
usuario=`whoami` #Variável que pega usuário logado para exibir abaixo em $usuario
echo  -e "$(tput setaf 10)[Bot assistant]:$(tput setaf 7) Olá $usuario, serei seu assistente para iniciar o SEER!\n"
echo  -e "$(tput setaf 10)[Bot assistant]:$(tput setaf 7)  Verificando aqui se você possui o Java instalado...\n"

java -version
if [ $? -eq 0 ]
	then
		echo "$(tput setaf 10)[Bot assistant]:$(tput setaf 7) : Olá você já tem o java instalado!"
		echo -e "Iniciando aplicação SEER ...\n"
		diretorio=`pwd`
		java -jar $diretorio/tela-login-inview-1.0-SNAPSHOT-jar-with-dependencies.jar
	else
		...
		then
			...
			if [ $VERSAO -eq 11 ]
				then
					...
					clear
					echo -e "$(tput setaf 10)[Bot assistant]:$(tput setaf 7) Java instalado com sucesso!\n"
					echo -e "Iniciando aplicação SEER ...\n"
					java -jar tela-login-inview-1.0-SNAPSHOT-jar-with-dependencies.jar
				fi
		else 	
		echo "$(tput setaf 10)[Bot assistant]:$(tput setaf 7)  Você optou por não instalar o Java por enquanto, até a próxima então!"
	fi
fi
#stop 10 (Comando estava dando erro, então comentei)
